package com.courier.commons.util.excel;
...
public class ExcelUtil {
     ...
     */
    public static <T> int exportExcel(SXSSFWorkbook workbook, SXSSFSheet sheet, int index, String[] headers, Collection<T> dataset, OutputStream out, byte type) {
        return exportExcel(workbook, sheet, index, headers, dataset, out, "yyyy-MM-dd", type);
    }
     ...
     */
    public static <T> int exportExcel(SXSSFWorkbook workbook, SXSSFSheet sheet, int index, String[] headers, Collection<T> dataset, OutputStream out,
                                      String pattern, byte type) {
        int i = write2Sheet(sheet, index, headers, dataset, pattern, type);
//        try {
//            workbook.write(out);
//        } catch (IOException e) {
...
//        }
     ...
     * @param out    与输出设备关联的流对象，可以将EXCEL文档导出到本地文件或者网络中
     */
    public static <T> void exportExcel(List<ExcelSheet<T>> sheets, int index, OutputStream out, byte type) {
        exportExcel(sheets, index, out, null, type);
    }
     ...
     * @param pattern 如果有时间数据，设定输出格式。默认为"yyy-MM-dd"
     */
    public static <T> void exportExcel(List<ExcelSheet<T>> sheets, int index, OutputStream out, String pattern, byte type) {
        ...
        for (ExcelSheet<T> sheet : sheets) {
            // 生成一个表格
            SXSSFSheet hssfSheet = workbook.createSheet(sheet.getSheetName());
            write2Sheet(hssfSheet, index, sheet.getHeaders(), sheet.getDataset(), pattern, type);
        }
        try {
            workbook.write(out);
        } catch (IOException e) {
     ...
     * @param pattern 日期格式
     */
    private static <T> int write2Sheet(SXSSFSheet sheet, int index, String[] headers, Collection<T> dataset,
                                       String pattern, byte type) {
        SXSSFRow row = null;
        // 产生表格标题行
        if (headers != null) {
        ...
        while (it.hasNext()) {
            ...
            try {
                if (t instanceof Map) {
                    ...
                    List<FieldForSortting> fields = sortFieldByAnno(t.getClass(), type);
                    int cellNum = 0;
...
//        }
    ...
    public static <T> Collection<T> importExcel(Class<T> clazz, InputStream inputStream,
                                                String pattern, ExcelLogs logs, byte type, Integer... arrayCount) {
        ...
        try {
            ...
            while (rowIterator.hasNext()) {
                ...
                if (clazz == Map.class) {
                    ...
                    List<FieldForSortting> fields = sortFieldByAnno(clazz, type);
     ...
     * @return
     */
    private static List<FieldForSortting> sortFieldByAnno(Class<?> clazz, byte type) {
