...
import com.courier.commons.constant.HttpConstant;
...
public class HttpUtils {
     ...
     * @return
     */
    public HttpRequestResult doHttpMethod(@NotNull HttpMethodType methodType, @NotNull String url, Map<String, String> header
    ...
    public HttpRequestResult doPost(String url, Map<String, String> header, Map<String, String> params, String bodyParam){
        return doPost(url, header, params, bodyParam, HttpConstant.HTTP_CONNECT_TIMEOUT, HttpConstant.HTTP_CONNECT_REQUEST_TIMEOUT, HttpConstant.HTTP_SOCKET_TIMEOUT);
    }
     ...
     * @return
     */
    public HttpRequestResult doPost(@NotNull String url, Map<String, String> header
            , Map<String, String> params, String bodyParam, Integer connectTimeout, Integer connectRequestTimeout, Integer socketTimeout){
     ...
     * @return
     */
    public HttpRequestResult doGet(String url, Map<String, String> header, Map<String, String> params, String bodyParam) {
    ...
    public HttpRequestResult doGet(String url) {
        return doGet(url, null, null, null, HttpConstant.HTTP_CONNECT_TIMEOUT, HttpConstant.HTTP_CONNECT_REQUEST_TIMEOUT, HttpConstant.HTTP_SOCKET_TIMEOUT);
    }
     ...
     * @return
     */
    public HttpRequestResult doGet(String url, Map<String, String> header, Map<String, String> params
    ...
    private HttpRequestResult execHttpMethod(@NotNull HttpMethodType type, @NotNull String url, Map<String, String> header , Map<String, String> params
        ...
        try {
            ...
            if (entity.isStreaming()) {
                ...
                byteCotent = EntityUtils.toString(entity, HttpConstant.DEFAULT_HTTP_CHARSET);
                requestResult.setContent("outputstream");

            } else {
                setStream(false);
                result.append(EntityUtils.toString(entity, HttpConstant.DEFAULT_HTTP_CHARSET));
                byteCotent = result.toString();
                requestResult.setContent(result.toString());
            }
            getHeader(response);
            EntityUtils.consume(entity);
//            logger.debug("url is {}, exec {}, method:{}, httpCode:{}, responseContent:{}", url, status, type.getName(), code, request.toString());
        } catch (SocketTimeoutException | ConnectTimeoutException e) {
            requestResult.setCode(HttpStatus.SC_REQUEST_TIMEOUT);
            logger.error("url is {}, exec {}, method:{}, httpCode:{}, cause:{}", url, status, type.getName(), HttpStatus.SC_REQUEST_TIMEOUT, e.getMessage());
        } catch (IOException e){
            requestResult.setCode(HttpStatus.SC_REQUEST_TIMEOUT);
            logger.error("url is {}, exec {}, method:{}, httpCode:{}, cause:{}", url, status, type.getName(), HttpStatus.SC_REQUEST_TIMEOUT, e.getMessage());
        } catch (Exception e){
            logger.error("url is {}, exec {}, method:{}, cause:{}", url, status, type.getName(), e.getMessage());
        } finally{
    ...
    private HttpRequestBase getHttpRequest(HttpMethodType type, String url, Map<String, String> header
        ...
        } else if(HttpMethodType.TRACE == type){
            request = new HttpTrace();
        } else if(HttpMethodType.OPTIONS == type){
            request = new HttpOptions();
        } else if(HttpMethodType.PATCH == type) {
    ...
    private void setGetParams(StringBuilder url, Map<String, String> params){
        if(params != null && !params.isEmpty()){
            try {
                ...
                logger.error("url is {}, params is {}, http get add params error, cause:{}", url, params, e.getMessage());
            }
        }
    }
